#!/usr/bin/env bash
# Shared exact-set coverage validator for triage-assess.yml (ported from automated-researcher PR #456 via
...
# returned ticket-number set exactly equals the gathered packets set (no omissions, duplicates, or invented
# numbers); on any mismatch it fails loudly with the diff and the caller's `set -e` takes down the run before
# adjudication (or posting) ever starts.
#
# Optional --check-wave: the semantic wave-assignment invariant — DO verdict requires an integer wave >= 1,
# SKIP/ASK verdict requires a null wave — checked the same fail-closed way, offending ticket numbers listed.
# Only the adjudication output carries a `wave` field at all (the blind-assessment schema has none), so only
# the adjudicate call site passes this flag.
#
