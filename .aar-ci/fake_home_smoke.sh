...
# die at command-not-found with its output swallowed by >/dev/null (automated-researcher#407, ported here
# as part of agentic-engineering#43's GitHub-native pipeline rollout, which runs this smoke on a GitHub
# runner that needs the CLI installed by a prior workflow step)
...
# available: this smoke must never silently no-op. (automated-researcher#396, ported here for the same
# reason as the command-v check above.)
