...
if printf '%s\n' "${PATHS[@]}" | grep -q '^plugins/aar-engineering/skills/ship-change/scripts/wf.sh$'; then
  ...
  GC_SMOKE="$ROOT/plugins/aar-engineering/skills/ship-change/scripts/worktree_gc_smoke.sh"
  if [ -f "$GC_SMOKE" ]; then
    echo "[checks] worktree gc smoke" >&2
    bash "$GC_SMOKE" >&2 && ok "worktree_gc smoke" || err "worktree_gc smoke FAILED"
  else
    err "wf.sh changed but worktree_gc_smoke.sh missing — cannot verify the abandoned-worktree sweep (#32)"
  fi
...
# 13. canonical-login smoke (agentic-engineering#43, ported from automated-researcher#381):
#     canonical_login() maps exactly the two GitHub-observed App-identity representations to the same form
#     WITHOUT collapsing a bare (untrusted) slug into matching the App, plus a static check that
#     implement-on-ready.yml sources the helper AFTER checkout (helper reachability from the real
#     workflow, not just unit correctness). Runs when the helper, its smoke, or either SWE-pipeline
#     workflow changed.
